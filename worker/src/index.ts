...
async function handleWebhook(request: Request, env: Env): Promise<Response> {
  ...
  if (!verifyWebhookSecret(env.TELEGRAM_WEBHOOK_SECRET, secret)) {
    console.error("Webhook secret mismatch", { got: secret?.slice(0, 8) });
  ...
  try {
    ...
    console.log(`Command: /${parsed.command} from ${telegramId}`);
    ...
    const sent = await sendMessage(env.TELEGRAM_BOT_TOKEN, chatId, reply);
    if (!sent) console.error("sendMessage failed", { chatId });
